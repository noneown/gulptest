...
var sass 			= require('gulp-ruby-sass'),			// CSS预处理/Sass编译
    uglify 			= require('gulp-uglify'),				// JS文件压缩 混淆压缩
    imagemin 		= require('gulp-imagemin'),		// imagemin 图片压缩
    pngquant 		= require('imagemin-pngquant'),	// imagemin 深度压缩 png
    ...
    autoprefixer 			= require('gulp-autoprefixer');				// 用来给css样式自动添加浏览器前缀
    minifycss 			= require('gulp-minify-css');				// css文件压缩
    notify 			= require('gulp-notify');				// 前台通知
...
gulp.task('sass', function () {
    return sass( srcPath.css + '/*.scss', { style: 'compact', sourcemap: true }) // 指明源文件路径、并进行文件匹配（编译风格：简洁格式）
        ...
        .pipe(gulp.dest(destPath.css)) // 输出路径 destPath.css
        .pipe(livereload())
...
gulp.task('script', function() {
    return gulp.src( [srcPath.script+'/*.js','!'+srcPath.script+'/*.min.js'] ) // 指明源文件路径、并进行文件匹配，排除 .min.js 后缀的文件
        ...
        .pipe(livereload())
        .pipe(gulp.dest(destPath.script)) // 输出路径  destPath.script
...
gulp.task('images', function(){
    return gulp.src( srcPath.image+'/**/*' ) // 指明源文件路径，如需匹配指定格式的文件，可以写成 .{png,jpg,gif,svg}
        ...
        .pipe(gulp.dest(destPath.image)); // 输出路径 destPath.image
});
// 文件合并
...
gulp.task('release', ['clean'], function(){ // 开始任务前会先执行[clean]任务
    return gulp.start('sassRelease','scriptRelease','images', 'html'); // 等[clean]任务执行完毕后再执行其他任务
});
